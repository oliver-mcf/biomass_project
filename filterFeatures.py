...
def matrix(csv_file, coef):
    ...
    for i in range(len(correlation_matrix.columns)):
        if correlation_matrix.columns[i] not in variables_to_remove:  # Skip already decided variables
            found_high_corr = False
            for j in range(i + 1, len(correlation_matrix.columns)):  # Only check upper triangular matrix
                if correlation_matrix.columns[j] not in variables_to_remove:  # Skip already decided variables
                    if abs(correlation_matrix.iloc[i, j]) >= coef:
                        # Mark only one variable for removal, keep the other
...
if __name__ == '__main__':
    ...
    if args.reduce:
        ...
        csv_list = ['/home/s1949330/Documents/scratch/diss_data/model/MGR_MODEL_INPUT.csv', 
                    '/home/s1949330/Documents/scratch/diss_data/model/TKW_MODEL_INPUT.csv']

...
R2_ <- 1 - sum((target - predictions)^2) / sum((target - mean(target))^2)
RMSE_ <- sqrt(mean((target - predictions)^2))
MAE_ <- mean(abs(target - predictions))
mean_target <- mean(target)
RMSE_percent_ <- (RMSE / mean_target) * 100
MAE_percent_ <- (MAE / mean_target) * 100

# Print the results
cat("R²:", R2_, "\n")
cat("RMSE (Mg/ha):", RMSE_, "\n")
cat("RMSE (%):", RMSE_percent_, "\n")
cat("MAE (Mg/ha):", MAE_, "\n")
cat("MAE (%):", MAE_percent_, "\n")

# Save the model
save(rf_model, file = "/home/s1949330/Documents/scratch/diss_data/model/RF_MODEL_LANDSAT.RData")
...
ggplot(segment_results, aes(x = Max_Target_Value)) +
  ...
  geom_hline(yintercept = RMSE_percent_, linetype = "dashed", color = "blue") +
  geom_hline(yintercept = MAE_percent_, linetype = "dashed", color = "red") +
  labs(x = "Predicted Landsat AGB", y = "Percentage of Mean", title = "Model Sensitivity to AGB Magnitude") +
  scale_color_manual(name = "Performance Statistic", values = c("RMSE%" = "blue", "MAE%" = "red")) +
  theme_classic() + 

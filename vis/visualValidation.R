...
# Read data
df <- read.csv('/home/s1949330/data/diss_data/model/yes_geo/All/predict/validate/BOTH_ALL_VALIDATION.csv', header = TRUE)
df_long <- df %>%
...
# Read data
df <- read.csv('/home/s1949330/data/diss_data/model/yes_geo/All/predict/avg_AGB.csv', header = TRUE)
df_long <- df %>%
...
# Read data
valid_df <- read.csv('/home/s1949330/data/diss_data/model/yes_geo/All/predict/validate/BOTH_ALL_VALIDATION.csv')
summary(valid_df)

# Plot scatter of field-gedi estimates
scatter <- ggplot(valid_df, aes(x = Field_AGB, y = GEDI_AGB, color = Site)) +
  geom_smooth(method = "lm", se = FALSE, color = "grey65", size = 0.75) +
  scale_color_manual(values = site_colors) +
  geom_point() +
  labs(x = "Field AGB Estimates (Mg/ha)", y = "Extrapolated GEDI AGB Estimates (Mg/ha)") +
  theme_classic() + 
  scale_y_continuous(limits = c(0, 120), breaks = seq(0, 120, by = 10)) +

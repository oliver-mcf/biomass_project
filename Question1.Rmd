...
plot_AGB <- lapply(unique(df_AGB$Site), function(site) {
  ...
  ggplot(df_site, aes(x = Month, y = AGB_Mean, color = Year_Category, group = Year_Category)) +
    geom_point(size = 2) +
    geom_line(aes(group = Year_Category, color = Year_Category), size = 1) +
    labs(title = paste("GEDI Aboveground Biomass, 2019-2022:", site), x = "Month", y = "AGB (Mg/ha)", color = "Legend") +
    scale_color_manual(values = c("2019-20" = "darkred", "2020-21" = "steelblue", "2021-22" = "orange")) +
    theme_classic() +
    theme(panel.grid.major = element_line(color = "grey95"),
          panel.grid.minor = element_line(color = "grey95"),
          panel.grid.minor.y = element_line(color = "gray95"))})
...
# Read data
site_data <- read.csv("/home/s1949330/Documents/scratch/diss_data/gedi/acf_analysis/data_TKW.csv")
site_data$Date <- as.Date(site_data$Date, format = "%d/%m/%Y")
biomass_ts <- ts(site_data$AGB_Mean)

# Plot ACF for biomass series
acf_values <- acf(biomass_ts, na.action = na.pass, lag.max = length(biomass_ts) + 1, plot = FALSE)
acf_values_plot <- acf_values$acf[-1]
summary(acf_values_plot)
plot(acf_values_plot, type = "h", main = "Autocorrelation of GEDI Biomass Data: TKW", xlab = "Lag (Months)", ylab = "Autocorrelation", xaxt = "n", ylim = c(min(acf_values_plot), max(acf_values_plot)))
axis(1, at = 1:length(biomass_ts), labels = 1:length(biomass_ts))
lines(1:length(acf_values_plot), acf_values_plot, col = "black")
abline(h = c(-1.96/sqrt(length(biomass_ts)), 1.96/sqrt(length(biomass_ts))), col = "blue", lty = 2)
abline(v = c(12, 24, 36), col = 'gray', lty = 2)
...
# Read data
site_data <- read.csv("/home/s1949330/Documents/scratch/diss_data/gedi/acf_analysis/data_TKW.csv")
x <- as.Date(site_data$Date, format = "%d/%m/%Y")
x_numeric <- seq_along(x)
biomass_ts <- ts(site_data$AGB_Mean)

# Derive AR statistics
AR <- arima(biomass_ts, order = c(1,0,0))
print(AR)
AR_model <- biomass_ts - residuals(AR)
print(AR_model)

# Plot AR model for biomass series
x_labels <- format(x, "%m/%y")
plot(x_numeric, biomass_ts, type = 'n', xlab = '', ylab = 'AGB (Mg/ha)', main = 'Auto-Regression Model of GEDI Biomass: TKW', ylim = c(0, 250), xaxt = 'n')
axis(1, at = x_numeric, labels = x_labels, las = 2)
mtext("Time", side = 1, line = 4)
abline(v = c(1, 12, 24, 36), col = 'gray', lty = 2)
points(x_numeric, biomass_ts, type = "o", col = 'black', pch = 16)
lines(x_numeric, AR_model, col = 'red', lty = 2)
legend("topleft", legend = c('GEDI Biomass', 'AR Model'), col = c('black', 'red'), lty = c(1, 2), pch = c(16, NA))

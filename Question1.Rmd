...
df <- df_biomass %>% filter(!is.na(AGB_Mean) & Site %in% c("MGR", "TKW"))
plot_biomass <- ggplot(df, aes(x = factor(Site), desc(AGB_Mean), y = AGB_Mean, fill = Site)) +
  ...
  labs(title = "GEDI AGB Estimates in Study Sites, 2019-2022", y = "\nBiomass (Mg/ha)", x = "\nSite") +
  guides(fill = "none", color = "none") +
  scale_fill_manual(values = c("#b22222", "#008000")) +
  scale_colour_manual(values = c("#b22222", "#008000")) +
  coord_flip() +
  theme_classic() + 
